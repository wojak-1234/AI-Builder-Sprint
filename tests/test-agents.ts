...
async function runTests() {
  console.log("=== 1. Testing OCR Extractor Agent ===");
  const testOcrText = "1972년 8월 15일 선우 돌잔치를 마당에서 했다. 삼촌이랑 이모가 와서 시끌벅적했다.";
  const ocrResult = await ocrExtractorAgent.extractEntities(testOcrText);
  ...
  console.log("Low confidence flag:", ocrResult.lowConfidence);
  ...
  console.log("\n=== 2. Testing Question Generator Agent ===");
  ...
  const allEndWithQuestionMark = generatedQuestions.every(q => q.text.endsWith("?"));
  console.log("All questions end with '?':", allEndWithQuestionMark);
  
  if (generatedQuestions.length > 0 && allEndWithQuestionMark) {
    console.log("✅ Question Generator Agent passed.");
  } else {
    console.error("❌ Question Generator Agent failed.");
  }

  console.log("\n=== 3. Testing Safety Guard Agent ===");
  // Test case A: Safe text
  const safeText = "그때 먹었던 김밥 참 맛있었는데 또 먹고 싶네요.";
  const safetyResultA = await safetyGuardAgent.checkSafety(safeText, "question");
  ...
  const safetyResultB = await safetyGuardAgent.checkSafety(unsafeText, "narrative");
  console.log("Unsafe text blocked (passed should be false):", safetyResultB.passed === false);
  console.log("Reason for block:", safetyResultB.reason);
  console.log("Safety Fallback Output:", safetyResultB.fallbackOutput);

  if (safetyResultA.passed && !safetyResultB.passed && safetyResultB.fallbackOutput) {
    console.log("✅ Safety Guard Agent passed.");
  } else {
    console.error("❌ Safety Guard Agent failed.");
  }

  console.log("\n=== 4. Testing Narrative Builder Agent (Chronological + Merged) ===");
  const testAnswers: DBAnswer[] = [
    {
      id: "ans-1",
      user_id: "user-elderly-123",
      question_id: "q-1",
      question_text: "어릴 적 마당에서 하던 놀이가 기억나나요?",
      answer_text: "마당 감나무 밑에서 동네 얘들이랑 술래잡기를 하고 꼬마들이 참 좋아했지.",
    ...
    },
    {
      ...
      question_id: "q-1",
      question_text: "어릴 적 마당에서 하던 놀이가 기억나나요?",
      answer_text: "엄마가 어렸을 때 한옥 기와집 감나무 마당에서 친구들과 홍시 따먹고 놀았대요.",
      created_at: new Date().toISOString(),
      event_date: "1955-10-15",
      is_private: false,
      by_guardian: true,
  ...
  const narratives = await narrativeBuilderAgent.buildNarratives("user-elderly-123", testAnswers);
  console.log("Narratives count:", narratives.length);
  console.log("Reconstructed Chapters:", JSON.stringify(narratives, null, 2));

  if (narratives.length > 0 && narratives[0].mergedAnswers && narratives[0].mergedAnswers.length > 0) {
